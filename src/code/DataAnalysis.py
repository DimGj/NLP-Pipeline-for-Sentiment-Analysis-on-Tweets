...
def DataAnalysis_ii(Data):
    MostCommon = imports.Counter(" ".join(Data["text"]).split()).most_common(10)
    MostCommonStr = []
    MostCommonValues = []
    MostCommonStr,MostCommonValues = FileHandlers.SplitTuple(MostCommon)
...
def DataAnalysis_iii(Data):
    MostCommonStr = []
    MostCommonValues = [] 
    Neg = Data.groupby('sentiment').get_group("NEG")
    Pos = Data.groupby('sentiment').get_group("POS")
    Neu = Data.groupby('sentiment').get_group("NEU")
    MostCommonNeg = imports.Counter(" ".join(Neg["text"]).split()).most_common(3)
    MostCommonPos = imports.Counter(" ".join(Pos["text"]).split()).most_common(3)
    MostCommonNeu = imports.Counter(" ".join(Neu["text"]).split()).most_common(3)
    MostCommonNegStr,MostCommonNegValues = FileHandlers.SplitTuple(MostCommonNeg)
    MostCommonPosStr,MostCommonPosValues = FileHandlers.SplitTuple(MostCommonPos)
    MostCommonNeuStr,MostCommonNeuValues = FileHandlers.SplitTuple(MostCommonNeu)
    ...
    Colors = ( "red","red","red","cyan","cyan","cyan","orange","orange","orange",)
...
def DataAnalysis_v(Data):
    Data['month'] = imports.pd.DatetimeIndex(Data['date']).month
...
def CreatePlot(ExplodeData,Colors,Values,StrArray,LegendPreview,LegendTitle):
    Fig,Ax = imports.plt.subplots(figsize=(12,9))
    ...
    imports.plt.setp(autotexts,size = 7,weight = "bold")
    imports.plt.title(LegendTitle,y=1.08)
